...
async function main() {
    ...
    const aluminium = prices.find((p) => p.symbol === 'AL');
    if (!copper || !aluminium) {
    ...
    for (let i = 1; i <= 50; i++) {
        ...
        if (res.success) {
            ...
            expectedUsd += res.order.totalUsd;
            if (metal.symbol === 'CU')
                expectedCu -= res.order.amountKg;
            else
                expectedAl -= res.order.amountKg;
            process.stdout.write(`${C.green}✓${C.reset}`);
        }
        else {
            stats.sells.fail++;
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        if (res.success) {
            ...
            if (metal.symbol === 'CU')
                expectedCu -= res.amountKg;
            else
                expectedAl -= res.amountKg;
    ...
    const actualUsd = wallets.wallets.find((w) => !w.metalId || w.type === 'usd')?.balance ?? 0;
    const actualCu = wallets.wallets.find((w) => w.metal?.symbol === 'CU')?.balance ?? 0;
    const actualAl = wallets.wallets.find((w) => w.metal?.symbol === 'AL')?.balance ?? 0;
    const invCu = inventory.find((i) => i.metal.symbol === 'CU');
    const invAl = inventory.find((i) => i.metal.symbol === 'AL');
    console.log(`\n${C.bold}${C.cyan}═══════════════════════════════════════════════════${C.reset}`);
